...
//            kppkSaved = savedInstanceState.getStringArrayList("kppkSaved");
    ...
    private void setUpLayout() {
        myLinearLayout = (LinearLayout)rootView.findViewById(R.id.container_komisiKaleb);
        imageLayout = (LinearLayout)rootView.findViewById(R.id.container_komisiKaleb_image);
    ...
    private void generateKontenUI (JSONArray arrData, JSONArray arrIsi) {
        setUpLayout();
        ...
        if (arrIsi != null) {
            ...
            try {
                ...
                ImageView GambarIV = (ImageView) rootView.findViewById(R.id.image_komisiKaleb);
        ...
        /*if (arrData != null) {
            ...
            for (int i = 0; i < dataLength; i++) {
                ...
                try {
                    ...
                    isiTempat = jsonobj.getString("tempat");
                    ...
                    for (int j = 0; j < length2; j++) {
                        ...
                        isiKebaktian = jsonArrAtr.getJSONObject(j).getString("kebaktian");
                        isiGedung = jsonArrAtr.getJSONObject(j).getString("gedung");
                        isiWaktuMulai = jsonArrAtr.getJSONObject(j).getString("waktumulai");
                        isiWaktuSelesai = jsonArrAtr.getJSONObject(j).getString("waktuselesai");
                        isiKelas = jsonArrAtr.getJSONObject(j).getString("kelas");
                        String kebaktianWaktu = null;

                        Log.d("Kaleb kebaktian", isiKebaktian);
                        Log.d("Kaleb gedung", isiGedung);
                        Log.d("Kaleb waktu mulai", isiWaktuMulai);
                        Log.d("Kaleb waktuSelesai", isiWaktuSelesai);
                        Log.d("Kaleb kelas", isiKelas);

                        if (kebaktianWaktu == kebaktian_tmp)
                            kebaktianWaktu = "";
                        else
                            kebaktianWaktu = isiKebaktian + " (" + isiWaktuMulai + "-" + isiWaktuSelesai + ")";
                        kebaktian_tmp = kebaktianWaktu;

                        fillTextViewHeader("Kebaktian");
                        fillTextView(isiKebaktian);

                        fillTextViewHeader("Kelas");
                        fillTextView(isiKelas);

                        fillTextViewHeader("Waktu");
                        fillTextView(isiWaktuMulai + "-" + isiWaktuSelesai);

                        fillTextViewHeader("Tempat");
                        fillTextView(isiTempat);

                        fillTextViewHeader("Gedung");
                        fillTextView(isiGedung);
                    }
                } catch (JSONException e) {
                    e.printStackTrace();
                }
